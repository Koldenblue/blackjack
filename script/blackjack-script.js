...
function Deck(playerCount) {
    ...
    this.dealAll = function () {
        ...
        for (var i = 0, j = this.cardsInDeck.length; i < j; i++) {
            var cardsRemaining = this.cardsInDeck.length;
            var dealtCardIndex = Math.floor(Math.random() * cardsRemaining);
            ...
            var dealtCard = this.cardsInDeck.splice(dealtCardIndex, 1);
    ...
    this.dealOne = function (currentPlayer, playerHands) {
        ...
        var currentHand = currentPlayer;
        var dealtCardIndex = Math.floor(Math.random() * this.cardsInDeck.length);
        var dealtCard = this.cardsInDeck.splice(dealtCardIndex, 1);
        playerHands[currentPlayer].push(dealtCard[0]);
        // Display the card:
        var graphicCard = document.createElement('p');
...
// child of Deck class:
function PlayingCards(playerCount) {
    if (playerCount === void 0) { playerCount = 4; }
...
var graphicHand = document.getElementById('graphicHand');
